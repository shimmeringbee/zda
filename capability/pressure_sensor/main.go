...
func (i *Implementation) Init(supervisor zda.CapabilitySupervisor) {
	...
	i.supervisor.ZCL().Listen(func(address zigbee.IEEEAddress, appMsg zigbee.ApplicationMessage, zclMessage zcl.Message) bool {
		...
		return zclMessage.ClusterID == zcl.PressureMeasurementId && canCast
	}, i.zclCallback)
}
...
func (i *Implementation) pollDevice(ctx context.Context, d zda.Device) bool {
	...
	results, err := i.supervisor.ZCL().ReadAttributes(ctx, d, endpoint, zcl.PressureMeasurementId, []zcl.AttributeID{pressure_measurement.MeasuredValue})
	if err == nil {
		if results[pressure_measurement.MeasuredValue].Status == 0 {
			i.setState(d, results[pressure_measurement.MeasuredValue].DataTypeValue.Value.(int64))
		}
	}
