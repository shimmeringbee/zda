...
import (
	...
	"github.com/shimmeringbee/zda/capabilities"
...
func TestProductInformation(t *testing.T) {
	t.Run("accepts data on attach and returns via Get", func(t *testing.T) {
		...
		attached, err := pi.Attach(nil, nil, catabilities.Enumeration, map[string]interface{}{
			"Name":         "NEXUS-7",
		...
		expectedInfo := dacapabilities.ProductInfo{
			Manufacturer: "Tyrell Corporation",
			Name:         "NEXUS-7",
			Serial:       "N7FAA52318",
			Version:      "",
	...
	t.Run("Capturing state and reloading should result in same output state", func(t *testing.T) {
		...
		attached, err := pi1.Attach(nil, nil, catabilities.Enumeration, map[string]interface{}{
			"Name":         "NEXUS-7",
			"Manufacturer": "Tyrell Corporation",
			"Serial":       "N7FAA52318",
			"Version":      "1.0.0",
		...
		attached, err = pi2.Attach(nil, nil, catabilities.Load, state)
		assert.True(t, attached)
		assert.NoError(t, err)
