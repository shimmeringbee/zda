...
func TestImplementation_Change(t *testing.T) {
	t.Run("querying for data returns error if device is not in store", func(t *testing.T) {
		...
		err := i.Change(ctx, device, 0.5, 500*time.Millisecond)

		assert.Error(t, err)
		assert.Equal(t, da.DeviceDoesNotBelongToGatewayError, err)
	})

	t.Run("sends MoveToLevelWithOnOff command to device", func(t *testing.T) {
		...
		capDev := zda.Device{Identifier: addr, Capabilities: []da.Capability{capabilities.LevelFlag}}
		...
		mockZCL.On("SendCommand", mock.Anything, capDev, endpoint, zcl.LevelControlId, &level.MoveToLevelWithOnOff{
			Level:          127,
			TransitionTime: 5,
		}).Return(nil)
