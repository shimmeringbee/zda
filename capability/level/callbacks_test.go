...
func TestImplementation_enumerateDeviceCallback(t *testing.T) {
	t.Run("removes capability if no endpoints have the Level cluster", func(t *testing.T) {
		mockAM := mocks.MockAttributeMonitor{}
		defer mockAM.AssertExpectations(t)
		...
		mockManageDeviceCapabilities.On("Remove", device, capabilities.LevelFlag)
		...
		assert.Equal(t, 0.0, i.data[addr].State)
	})

	t.Run("adds capability and sets product data if on first endpoint that has Level cluster, puts requires polling in data", func(t *testing.T) {
		mockAM := mocks.MockAttributeMonitor{}
		defer mockAM.AssertExpectations(t)
		...
		mockManageDeviceCapabilities := mocks.MockManageDeviceCapabilities{}
		defer mockManageDeviceCapabilities.AssertExpectations(t)
		mockManageDeviceCapabilities.On("Add", device, capabilities.LevelFlag)
